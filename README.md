...
本项目**绝大部分**的开发工作由AI完成，仅由我进行需求描述、代码审查和整合。
...
状态面板支持忘记密码重置。首次登录后建议在账户设置中填写找回邮箱，并在系统配置中完成 SMTP 邮件配置与测试；之后如果忘记密码，可以在登录页点击“忘记密码？”，输入预设的找回邮箱。系统会校验邮箱是否匹配，匹配后自动生成新密码并发送到该邮箱。

**注意**：忘记密码功能依赖 SMTP 和找回邮箱。未配置 SMTP、SMTP 不可用，或未提前设置找回邮箱时，系统无法通过邮件重置账户凭据。
...
例如把内网 SSH 服务映射到 VPS 的 `19322` 端口：
